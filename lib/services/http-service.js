...
function httpServiceFactory(
    ...
    HttpService.prototype.start = function() {
        var self = this;
        if (self.endpoint.httpsEnabled) {
            self.server = https.createServer(self.httpsOptions, self.app);

            wss.start(self.server, {
                secureOptions: self.httpsOptions
            });
        } else {
        ...
        return self.server.listen(self.endpoint.port, self.endpoint.address);
