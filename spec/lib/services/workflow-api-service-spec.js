...
describe('Http.Services.Api.Workflows', function () {
    ...
    it('should throw error, when cancelling an non-active workflow ', function () {
        var mockWorkflowError = new Errors.TaskCancellationError("testid is not an active workflow");
        waterline.graphobjects.needByIdentifier.rejects(mockWorkflowError);
        return workflowApiService.cancelTaskGraph().should.eventually.be.rejectedWith(mockWorkflowError);
    });
